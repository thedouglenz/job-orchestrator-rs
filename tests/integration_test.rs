...
use database::{
    Database, DatabaseConfig, PostgresImageRegistryRepository,
...
use std::sync::Arc;
use types::{ExecutorConfig, JobRequest, JobStatus};
use uuid::Uuid;
...
async fn test_full_job_lifecycle() {
    with_test_database(|db| async move {
        let job_repo: Arc<dyn JobQueueRepository> =
            Arc::new(PostgresJobQueueRepository::new(Arc::clone(&db)));
        let image_repo: Arc<dyn ImageRegistryRepository> =
            Arc::new(PostgresImageRegistryRepository::new(Arc::clone(&db)));
        let history_repo: Arc<dyn JobExecutionHistoryRepository> =
            Arc::new(PostgresJobExecutionHistoryRepository::new(Arc::clone(&db)));

        // Create test image in registry
        let image_id = Uuid::new_v4().to_string();
        create_test_image(&db, &image_id, "test-image", "test-repo", "latest")
            .await
...
async fn test_topic_filtering() {
    with_test_database(|db| async move {
        ...
        let job_id1 = job_repo
            .enqueue_job(&job1)
            .await
            .expect("Failed to enqueue job1");
        let job_id2 = job_repo
            .enqueue_job(&job2)
            .await
            .expect("Failed to enqueue job2");

        // Verify jobs are enqueued
        let _job1_status = job_repo
            .get_job(&job_id1)
            .await
            .expect("Failed to get job1");
        let _job2_status = job_repo
            .get_job(&job_id2)
            .await
            .expect("Failed to get job2");
    ...
    with_test_database(|db| async move {
        ...
        for topic in &topics {
            ...
            let job_id = job_repo
                .enqueue_job(&job)
                .await
                .expect(&format!("Failed to enqueue job for {}", topic));
            job_ids.push(job_id);
        }
